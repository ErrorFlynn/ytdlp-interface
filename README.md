...
Next, you will need to build the depencencies. All of them have a solution file (`.sln`), except for libjpeg-turbo which uses CMake. In Visual Studio's "Get started" window, select "Open a folder", then select the folder `libjpeg-turbo-3.1.2`. You can then press F7 to build. Initially there is only the `x64-Debug` configuration, but you can add the rest by selecting `Manage configurations...` from the dropdown menu.
